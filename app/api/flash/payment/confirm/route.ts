...
import { z } from 'zod'
import { db } from '@/db'
import { flashPaymentIntents, flashCredits, flashEvents } from '@/db/schema'
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!authenticated || !wallet) {
      logger.warn('Unauthenticated flash payment confirm request')
    ...
    logger.info('Flash payment confirmation request', { intentId, transactionId: rawTransactionId, address })
    
    const transactionId = rawTransactionId.toLowerCase()

    const result = await db.transaction(async (tx) => {
      const lockResult = await tx.execute(
        sql`SELECT * FROM flash_payment_intents WHERE id = ${intentId} FOR UPDATE`
      )
      
      const rows = lockResult.rows as any[]
      if (!rows || rows.length === 0) {
        logger.error('Flash intent not found in database', { intentId, address })
      ...
      const intent = {
        ...
        flashEventId: rows[0].flash_event_id,
      ...
      logger.info('Flash intent found', { intentId, intentAddress: intent.address, requestAddress: address, status: intent.status })
      ...
      if (intent.status === 'confirmed') {
        logger.info('Flash intent already confirmed', { intentId })
        return { success: true, message: 'Credit already granted', flashEventId: intent.flashEventId }
      }
      ...
      const replayCheck = await tx
        .select()
        .from(flashPaymentIntents)
        .where(and(
          eq(flashPaymentIntents.txHash, transactionId),
          ne(flashPaymentIntents.id, intentId)
        ))
        .limit(1)
      ...
      if (!onChainVerification.valid) {
        logger.warn('On-chain verification failed for flash payment', { 
          intentId, 
          transactionId, 
      ...
      logger.info('On-chain verification passed for flash payment', { intentId, transactionId })

      try {
        await tx
          .update(flashPaymentIntents)
          .set({ status: 'confirmed', txHash: transactionId })
          .where(eq(flashPaymentIntents.id, intentId))
      } catch (dbError: any) {
      ...
      const existingCredits = await tx
        ...
        .where(and(
          ...
          eq(flashCredits.flashEventId, intent.flashEventId)
        ))
        .for('update')
        .limit(1)
      ...
      if (existingCredits.length > 0) {
        ...
        logger.info('Flash payment confirmed - credit incremented', { 
          intentId, address, flashEventId: intent.flashEventId, 
          transactionId, previousBalance: currentBalance, newBalance: currentBalance + 1 
        })
      } else {
        await tx.insert(flashCredits).values({
          address: address,
          flashEventId: intent.flashEventId,
          balance: 1,
          used: false
        })
        logger.info('Flash payment confirmed - new credit granted', { intentId, address, flashEventId: intent.flashEventId, transactionId })
      }

      return {
        success: true,
        message: 'Flash credit granted',
        flashEventId: intent.flashEventId,
        hasCredit: true
      }
    })
    ...
    if ('error' in result) {
      logger.warn('Flash payment rejected', { intentId, error: result.error, message: result.message })
    ...
    logger.error('Flash payment error', { error: String(error) })
